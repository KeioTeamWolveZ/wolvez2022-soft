...
class SPM2Evaluate():  # 藤井さんの行動計画側に移設予定
    def start(self, model_master, test_data_list_all_win, test_label_list_all_win, scaler_master,score_master_mother,geta=[0,0,0,0,0,0]):
    ...
    def get_score(self):
        self.score_master=(np.array(self.score_master)+np.array(self.geta)).tolist()
        return self.score_master,self.geta
